...
//ajout dans la base
    ...
    public function getRefByBenAction(Request $request){
        $ben = $request->get("benevole");
        $entities = $this->getDoctrine()->getRepository(Refugee::class)->getRefByBen($ben);
        $data = $this->get('jms_serializer')->serialize($entities, 'json');
        $response = new Response($data);
        return $response;
    }
    public function getRefIsDoneAction(){
        $em = $this->getDoctrine()->getManager();
        $Refugees = $em->getRepository(Refugee::class)->getRefIsDone();
        $data = $this->get('jms_serializer')->serialize($Refugees, 'json');
        $response = new Response($data);
        return $response;
    }
